...
class Grafo():
    ...
    def adicionar_vertice(self, cidade):
        self.lado = self.lado + (cidade,)

    def adjacencias(self, cidade):
        adjacente=tuple()
        for i in self.arco:
            if cidade in i.vertices:
                adjacente=adjacente+(i.oposto(cidade),)
        return adjacente
    ...
    def calcular_melhores_caminhos_partindo_de(self, cidade):
        ...
        Arcoslist = []
        visitados = [cidade]
        caminho[cidade] = (0, [cidade])
        for x in range(len(self.arco)):
            if cidade in self.arco[x].vertices:
                Arcoslist.append((self.arco[x].valor, self.arco[x]))
        while Arcoslist:
            for x in Arcoslist:
                if x[1].vertices[0] in visitados and x[1].vertices[1] in visitados:
                    Arcoslist.remove(x)
            Arcoslist.sort(key=lambda arcos: arcos[0])
            if Arcoslist:
                arco = Arcoslist.pop(0)
            else:
                return caminho
            ...
            caminho[visitados[-1]] = arco[0], caminho[cidade_vizinha][1] + [arco[1].valor] + [visitados[-1]]
            ...
            for posicao in range(len(self.arco)):
                if visitados[-1] in self.arco[posicao].vertices and (self.arco[posicao].vertices[0] not in visitados or self.arco[posicao].vertices[1] not in visitados):
                    Arcoslist.append((caminho[visitados[-1]][0] + self.arco[posicao].valor, self.arco[posicao]))
        return caminho

    def arcos(self, cidade):
        arco = tuple()
        for cont in self.arco:
            if cidade in cont.vertices:
                arco = arco + (cont,)
        return arco
    
    def caminho(self, n1, n2):
        y = []
        if n1 == n2:
            y.append(n1)
            return y
        else:
            if self.arcos(n1) == tuple():
                return y
            else:
                x = n1
                while x is not n2:
                    if isinstance(self.arcos(x), tuple()):
                        break;
                    for i in self.arcos(x):
                        if i.oposto(x) not in y:
                            y.append(x)
                            x = i.oposto(x)
                            break;
                y.append(x)
...
# Dados a serem usados nos testes (caminho mais curto, a.k.a. algoritmo de Dijkstra
